...
function updateTitlebar() {
  try {
    ...
    const c = incog ? { color: '#0b0618', symbolColor: '#cbbde6' } : dark ? { color: '#0d0912', symbolColor: '#d4d4d8' } : { color: '#e7edf4', symbolColor: '#0e2a47' };
...
async function refreshPro() {
  ...
  try {
    ...
    // "no_session" kan vara en tillfällig serverhicka — vi behåller kontot lokalt tills användaren själv loggar ut.
  } catch {}
...
function renderSetHero() {
  ...
  if (!account) {
    ...
    nm.textContent = 'Skugga'; em.textContent = 'Inte inloggad';
  ...
  const pic = (typeof socMe !== 'undefined' && socMe && socMe.avatar) ? socMe.avatar : null;
  av.innerHTML = pic ? '<img src="' + pic + '" alt="">' : escapeHtml((name[0] || '?').toUpperCase());
  nm.textContent = name;
  em.textContent = account.isChild ? 'Barnkonto' : (account.email || '');
  const badge = $('kn-badge'); const pro = badge ? badge.textContent.trim() : 'Pro';
...
// Skugga: inga konton — inloggning borttagen för maximal anonymitet. Återställ ALDRIG ett konto, och töm ev. gammalt.
account = null;
try { localStorage.removeItem('skoll-account'); } catch {}
...
function updateAccountBtn() {
  $('account-btn').classList.toggle('on', !!account); greet(); if (typeof applyAuthGates === 'function') applyAuthGates();
...
function openLogin() {
  // Skugga: inloggning borttagen för maximal anonymitet — login-panelen öppnas aldrig.
  return;
}
function closeLogin() {
  if (active) active.overlay = null;
...
async function famSyncParent() {
  ...
  if (r && r.ok && Array.isArray(r.children)) {
    ...
    fam.children = r.children.map((c) => ({ id: c.id, name: c.name, age: c.age, blocklist: Array.isArray(c.blocklist) ? c.blocklist : [], code: c.code }));
...
if (account && account.token) { try { window.session.setkey(accountKey(account)); } catch {} if (account.isChild) { syncChild(); startChildSync(); } else refreshPro(); }
else { try { window.session.setkey(null); } catch {} }

/* ── Tor: uppkopplingslås + statuspill + ny identitet ─────────────────────── */
(function initTor() {
  const gate = document.getElementById('torgate');
  const bar = document.getElementById('torgate-bar');
  const pct = document.getElementById('torgate-pct');
  const errEl = document.getElementById('torgate-err');
  const sub = document.querySelector('.torgate-sub');
  const pill = document.getElementById('tor-pill');
  const pillTxt = document.getElementById('tor-pill-txt');
  if (!window.tor) return;
  ...
  function render(s) {
    ...
    if (s.phase === 'ready') {
      if (gate && !gate.classList.contains('hide')) { gate.classList.add('hide'); setTimeout(() => { gate.style.display = 'none'; }, 550); }
      if (pill) { pill.classList.remove('connecting', 'error'); pill.classList.add('ready'); }
      if (pillTxt) pillTxt.textContent = 'Tor';
    } else if (s.phase === 'error') {
      if (sub) sub.textContent = 'Kunde inte koppla upp mot Tor';
      if (errEl) errEl.textContent = s.error || 'Okänt fel — starta om Skugga.';
      if (pill) { pill.classList.remove('connecting', 'ready'); pill.classList.add('error'); }
      if (pillTxt) pillTxt.textContent = 'Tor av';
    } else {
      if (gate) { gate.style.display = ''; gate.classList.remove('hide'); }
      if (pill) { pill.classList.remove('ready', 'error'); pill.classList.add('connecting'); }
      if (pillTxt) pillTxt.textContent = p + '%';
    }
  }
  ...
  if (pill) pill.addEventListener('click', async () => {
    ...
    const old = pillTxt ? pillTxt.textContent : '';
    if (pillTxt) pillTxt.textContent = 'Ny…';
    const r = await window.tor.newIdentity().catch(() => ({ ok: false }));
    if (pillTxt) pillTxt.textContent = old || 'Tor';
    if (typeof showToast === 'function') showToast(r && r.ok ? 'Ny Tor-identitet — nytt utgångs-IP' : 'Kunde inte byta identitet');
  });
})();
